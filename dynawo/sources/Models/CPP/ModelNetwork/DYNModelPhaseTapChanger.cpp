...
bool
ModelPhaseTapChanger::getIncreaseTap(bool P1SupP2) const {
...
ModelPhaseTapChanger::evalG(double t, double iValue, bool /*nodeOff*/, state_g* g, double disable, double locked, bool tfoClosed) {
  ...
  g[2] = (moveUp_ && (t - whenUp_ >= getTFirst()) && getCurrentStepIndex() < getHighStepIndex() && !(locked > 0.) && getRegulating() &&
          getCurrentStepIndex() == tapRefUp_ && tfoClosed)
             ? ROOT_UP
             : ROOT_DOWN;  // first tap Up
  g[3] = (moveUp_ && (t - whenLastTap_ >= getTNext()) && getCurrentStepIndex() < getHighStepIndex() && !(locked > 0.) && getRegulating() &&
          getCurrentStepIndex() != tapRefUp_ && tfoClosed)
             ? ROOT_UP
             : ROOT_DOWN;  // next tap Up

  g[4] = (moveDown_ && (t - whenDown_ >= getTFirst()) && getCurrentStepIndex() > getLowStepIndex() && !(locked > 0.) && getRegulating() &&
          getCurrentStepIndex() == tapRefDown_ && tfoClosed)
             ? ROOT_UP
             : ROOT_DOWN;  // first tap down
  g[5] = (moveDown_ && (t - whenLastTap_ >= getTNext()) && getCurrentStepIndex() > getLowStepIndex() && !(locked > 0.) && getRegulating() &&
          getCurrentStepIndex() != tapRefDown_ && tfoClosed)
             ? ROOT_UP
