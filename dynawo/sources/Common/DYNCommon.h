...
fromNativeBool(const bool value) {
...
/**
   ...
   */
static inline bool
doubleEquals(const double& a, const double& b) {
  if (std::isinf(a) || std::isinf(b))
    return false;
  if (std::isnan(a) || std::isnan(b))
    return false;
  double diff = std::fabs(a - b);
  if (diff <= getCurrentPrecision() / 5)
    return true;
...
/**
   ...
   */
static inline bool
doubleNotEquals(const double& a, const double& b) {
...
/**
   ...
   */
static inline bool
toNativeBool(const double& dynawoBool) {
