 ...
 */
typedef enum { NoChange = 0x00, ModeChange = 0x01, ZChange = 0x02, SilentZChange = 0x04 } StateFlags;

/**
 * @brief Status of the previous reinitialization scheme
 */
typedef enum { None = 0, Algebraic, AlgebraicWithJUpdate } PreviousReinit;

/**
 * @brief Identifier of the current solver used
 */
typedef enum { SolverSimplifie = 0, SolverSundials1 = 1, SolverSundials2 = 2 } SolverType;
...
class Solver : private boost::noncopyable {
 ...
 protected:
   ...
   */
  template<typename T>
  void setParameterValue(ParameterSolver &parameter, const T &value);
