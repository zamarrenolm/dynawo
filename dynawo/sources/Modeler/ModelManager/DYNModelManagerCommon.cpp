...
void
printLogToStdOut_(ModelManager* model, const std::string& message) {
...
void
printLogExecution_(ModelManager* model, const std::string& message) {
  SubModel* sub = dynamic_cast<SubModel*>(model);
  if (sub == NULL)
    throw DYNError(Error::GENERAL, WrongDynamicCast);
...
void
addLogEvent_(ModelManager* model, const MessageTimeline& messageTimeline) {
  model->addEvent(model->name(), messageTimeline);
}

void
addLogEventRaw2_(ModelManager* model, const char* message1, const char* message2) {
  std::stringstream ss("");
...
void
addLogEventRaw3_(ModelManager* model, const char* message1, const char* message2, const char* message3) {
  std::stringstream ss("");
...
void
addLogEventRaw4_(ModelManager* model, const char* message1, const char* message2, const char* message3, const char* message4) {
  std::stringstream ss("");
...
void
addLogEventRaw5_(ModelManager* model, const char* message1, const char* message2, const char* message3, const char* message4, const char* message5) {
  std::stringstream ss("");
...
void
addLogConstraintBegin_(ModelManager* model, const Message& message) {
  model->addConstraint(model->name(), true, message);
}

void
addLogConstraintEnd_(ModelManager* model, const Message& message) {
  model->addConstraint(model->name(), false, message);
}

void
assert_(ModelManager* model, const Message& message) {
  throw MessageError(model->name() + " : " + message.str());
}

void
throw_(ModelManager* model, const Message& message) {
  throw MessageError(model->name() + " : " + message.str());
}

void
terminate_(ModelManager* model, const MessageTimeline& messageTimeline) {
  model->addEvent(model->name(), messageTimeline);
  throw DYNTerminate(TerminateInModel, model->name(), messageTimeline.str());
}

const char*
modelica_integer_to_modelica_string(modelica_integer i, modelica_integer /*minLen*/, modelica_boolean /*leftJustified*/) {
...
const char*
cat_modelica_string(modelica_string_const s1, modelica_string_const s2) {
  std::stringstream ss("");
  ss << s1 << s2;
  const std::string tmp = ss.str();
  mmChars.string2Keep_.push_back(tmp);
...
const char*
cat_modelica_string(std::string s1, modelica_string_const s2) {
  std::stringstream ss("");
  ss << s1 << s2;
  const std::string tmp = ss.str();
  mmChars.string2Keep_.push_back(tmp);
...
const char*
cat_modelica_string(modelica_string_const s1, std::string s2) {
  std::stringstream ss("");
  ss << s1 << s2;
  const std::string tmp = ss.str();
  mmChars.string2Keep_.push_back(tmp);
...
const char*
stringAppend(const modelica_string s1, const modelica_string s2) {
  std::stringstream ss("");
  ss << s1 << s2;
  const std::string tmp = ss.str();
  mmChars.string2Keep_.push_back(tmp);
...
const char*
stringAppend(const modelica_string s1, const std::string s2) {
  std::stringstream ss("");
  ss << s1 << s2;
  const std::string tmp = ss.str();
  mmChars.string2Keep_.push_back(tmp);
...
const char*
stringAppend(const std::string s1, const modelica_string s2) {
  std::stringstream ss("");
  ss << s1 << s2;
  const std::string tmp = ss.str();
  mmChars.string2Keep_.push_back(tmp);
...
std::string
mmc_strings_len1(unsigned int size) {
  const std::string tmp(size, '\0');
  return tmp;
}

const char*
modelica_real_to_modelica_string_format(modelica_real r, std::string /*format*/) {
  std::stringstream ss("");
...
const char*
modelica_integer_to_modelica_string_format(modelica_integer i, std::string /*format*/) {
  std::stringstream ss("");
  ss << i;
  const std::string tmp = ss.str();
  mmChars.string2Keep_.push_back(tmp);
...
const char*
modelica_real_to_modelica_string(modelica_real r, modelica_integer /*minLen*/, modelica_boolean /*leftJustified*/, modelica_integer signDigits) {
  // @todo warning: no thread safe
  std::stringstream ss("");
...
const char*
modelica_boolean_to_modelica_string(modelica_boolean b, modelica_integer /*minLen*/, modelica_boolean /*leftJustified*/) {
  if (b) {
    return "true";
  } else {
    return "false";
...
bool
compareString_(const std::string& x, const std::string& y) {
  return (x.compare(y) == 0);
}

const modelica_integer*
integerArrayElementAddress1_(const modelica_integer* source, int dim1) {
  return (source + dim1 - 1);
}

modelica_string
enumToModelicaString_(modelica_integer nr, const char* e[]) {
  return e[nr - 1];
}

modelica_integer
sizeOffArray_(const modelica_integer array[], modelica_integer dim) {
...
callExternalAutomatonModel(const std::string& modelName, const char* command, const double time, const double* inputs, const char** inputsName,
                           const int nbInputs, const int nbMaxInputs, double* outputs, const char** outputsName, const int nbOutputs, const int nbMaxOutputs,
                           const std::string& workingDirectory) {
  if (nbInputs >= nbMaxInputs)
    throw DYNError(Error::GENERAL, AutomatonMaximumInputSizeReached, modelName, boost::lexical_cast<std::string>(nbInputs),
                   boost::lexical_cast<std::string>(nbMaxInputs));
  if (nbOutputs >= nbMaxOutputs)
    throw DYNError(Error::GENERAL, AutomatonMaximumOutputSizeReached, modelName, boost::lexical_cast<std::string>(nbOutputs),
