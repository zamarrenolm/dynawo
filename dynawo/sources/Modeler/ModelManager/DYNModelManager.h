...
class ModelManager : public SubModel, private boost::noncopyable {
 ...
 private:
   ...
   */
  void setYCalculatedParameters(std::vector<double>& y, const std::map<std::string, std::vector<boost::shared_ptr<VariableAlias> > >& reversedAlias);
   ...
   * @param reversedAlias reference variable name to alias variables map
   */
  void setZCalculatedParameters(std::vector<double>& z, const std::map<std::string, std::vector<boost::shared_ptr<VariableAlias> > >& reversedAlias);
