...
void
Modeler::replaceStaticAndNodeMacroInVariableName(const shared_ptr<SubModel>& subModel1, string& var1, const shared_ptr<SubModel>& subModel2,
                                                 string& var2) const {
...
void
Modeler::replaceNodeWithBus(const shared_ptr<SubModel>& subModel1, string& var1, const shared_ptr<SubModel>& subModel2, string& var2,
                            const string& labelNode) const {
...
Modeler::initConnects() {
  ...
  for (std::map<string, shared_ptr<ConnectInterface> >::const_iterator itConnector = connects.begin(); itConnector != connects.end(); ++itConnector) {
...
void
Modeler::collectAllInternalConnections(shared_ptr<dynamicdata::ModelicaModel> model, vector<std::pair<string, string> >& variablesConnectedInternally) const {
...
Modeler::SanityCheckFlowConnection() const {
  ...
  for (std::map<string, shared_ptr<ConnectInterface> >::const_iterator itConnector = connects.begin(); itConnector != connects.end(); ++itConnector) {
    ...
    for (size_t i = 0, iEnd = connectedVars.size(); i < iEnd; ++i) {
      ...
      if (flowVarId2ConnIndex.find(firstVar) != flowVarId2ConnIndex.end()) {
        throw DYNError(Error::MODELER, FlowConnectionMixedSystemAndInternal, id1, connectedVars[i].first, id2, connectedVars[i].second);
      }
